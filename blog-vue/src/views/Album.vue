                ...
                <el-row class="header">
                    <router-link :to="'/AlbumDetail/' + authorId + '_' + item.albumName" class="item"
                        ...
                        <template v-if="isAuthor" >
                                    <el-tag v-if="item.isPublic==1" type="success" style="opacity:0.8">公开</el-tag>
                ...
                <el-row class="body">
                    <div class="item" v-for="(item, index) in simplifyImg" :key="index">
                        <el-image class="photo" :src="item" fit="cover" @click="previewImage(index)"
                            :preview-src-list="previewImg" :initial-index="0" preview-teleported />
                    </div>
                </el-row>
            </el-col>
            <el-col class="space"></el-col>
...
export default {
    ...
    methods: {
        previewImage(index) {
            this.previewImg[0] = this.originalImg[index];
        },
        // 上传图片部分
        ...
        async uploadFunc(e) {
            return new Promise((resolve) => {
                ...
                reader.onload = async (re) => {
                    ...
                    await API.post("image/add", data, (progressEvent) => {
                        e.percentage =
        ...
        saveEditAlbum() {
            ...
            setTimeout(() => {
                if (this.OKFlag) {
                    API.post("album/update", data).then((res) => {
                        if (res.code == 200) {
                            ElMessage({
                                showClose: true,
                                message: "编辑成功",
        ...
        editAlbumUploadHandler(file, index) {
            ...
            reader.onload = async (e) => {
                this.albumsA[index].coverImg = await this.dealImage(
                    e.target.result,
                    1000
                );
        ...
        initAlbums() {
            ...
            let api = (this.isAuthor)?("album/getAlbums"):("init/getPublicAlbums")
